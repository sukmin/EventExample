	...
	public static void raise(Object event) {
		...
		try {
			...
			if (asyncEventHandlers != null) {
				for (EventHandler aEventHandler : asyncEventHandlers) {
					if (aEventHandler.canHandle(event)) {
						executor.submit(() -> aEventHandler.handle(event));
					}
				}
			}
			...
			for (EventHandler aEventHandler : eventHandlers) {
				if (aEventHandler.canHandle(event)) {
